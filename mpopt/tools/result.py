...
def parsing():
    ...
    parser.add_argument(
        "--precision", "-p", default="1e-5", type=float, help="Precision in comparation"
    )
    parser.add_argument(
        "--alpha",
        "-a",
...
def stats_compare(*paths, benchmark=None, **kwargs):
    ...
    err1[err1 < precision] = 0
    err2[err2 < precision] = 0 
...
def average_rank(*paths, benchmark=None, **kwargs):
    ...
    for alg_idx in range(num_alg):
        print("{}: {}".format(names[alg_idx], ave_rank[alg_idx]))
