...
class IPOP_FWASSP(object):
    ...
    def optimize(self, e):
        ...
        while not e.terminate():
            ...
            min_fit = 1e10
            num_stop = 0
            #print("New Round: eval {}, fit {}".format(e.num_eval, e.cur_y))

            while not e.terminate():
                
                #print("New Iter: eval {}, fit {}, pop {}".format(e.num_eval, e.cur_y, self.fw_size))
                ...
                # restart population
                new_min_fit = min([fw.val for fw in self.fireworks])
                if min_fit - new_min_fit > 1e-6:
                    min_fit = new_min_fit
                    num_stop = 0
                else:
                    num_stop += 1
                    if num_stop > self.max_not_improved:
                        break
    ...
    def init(self, e):
        ...
        init_fit = e(init_pop)
        self.init_scale = (self.ub - self.lb) / self.fw_size
        self.spk_size = int(self.sp_size / self.fw_size)
        self.adapt_lr = 0.5
    ...
    def cooperate(self):
        ...
        for i in range(self.fw_size):
            for j in range(self.fw_size):
                if i == j: continue
                pj = self.fireworks[i].new_mvnd.pdf(poss[j,:])
        ...
        for i in range(self.fw_size):
            ...
            new_cov += (1.0 - lr + lr * np.mean(a) - EPS) * fw.new_mvnd.cov
