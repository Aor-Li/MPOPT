...
class BasePop(object):
    ...
    def remap(self, samples):
        """ Default: apply random_map on out-bounded samples """
...
class BaseFirework(object):
    ...
    def __init__(self, idv, val, dim=None, lb=-float('inf'), ub=float('inf')):
        # init pop
        self.idv = idv
        self.val = val
        self.spk_pop = None
        ...
        # params
        self.dim = dim if idv is None else self.idv.shape[0]
        self.lb = lb
        self.ub = ub
...
class BaseEDAFirework(object):
    """ Base Class for Fireworks """
    def __init__(self, dist, lb=-float('inf'), ub=float('inf')):
        # init 
        self.dist = dist
